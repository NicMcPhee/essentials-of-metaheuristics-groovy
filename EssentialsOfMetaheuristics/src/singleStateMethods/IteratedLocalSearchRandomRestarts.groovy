...
class IteratedLocalSearchRandomRestarts {
	...
	def maximize(problem) {
		...
		while (!problem.terminate(s, sQuality)) {
			//def time = t.sample
			while(!time.terminate || !problem.terminate(s, sQuality)){ //should this be based on s or best?
			...
			home = problem.newHomeBase(home, s) //Include homeQuality & sQuality?
